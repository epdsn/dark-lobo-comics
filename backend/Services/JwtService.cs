...
public class JwtService : IJwtService
{
    ...
    public string GenerateToken(User user)
    {
        ...
        var token = new JwtSecurityToken(
            issuer: _configuration["Jwt:Issuer"] ?? "DarkLoboComicsApi",
            audience: _configuration["Jwt:Audience"] ?? "DarkLoboComicsApp",
            claims: claims,
    ...
    public ClaimsPrincipal? ValidateToken(string token)
    {
        ...
        try
        {
            var principal = tokenHandler.ValidateToken(token, new TokenValidationParameters
            {
                ...
                ValidateIssuer = true,
                ValidIssuer = _configuration["Jwt:Issuer"] ?? "DarkLoboComicsApi",
                ValidateAudience = true,
                ValidAudience = _configuration["Jwt:Audience"] ?? "DarkLoboComicsApp",
